...
public class Gapminder : MonoBehaviour 
{
	...
	public static float POINT_SIZE = 1f;
	public static float VISUALIZATION_SIZE = 100000f;
	...
	void Start () {
		...
		dataPoints = d4.selectAll()
			.data(countries)
			.append("sphere")
				.attr("r", (d,i) => Mathf.Sqrt(d4.map( minPopulation, maxPopulation, .5f, 10, getFloat(d, "population", year))))
				.attr("x", (d,i) => Mathf.Log(d4.map( minGDP, maxGDP, 0f, VISUALIZATION_SIZE, getFloat(d, "gdp", year))))
				.attr("y", (d,i) => Mathf.Log(d4.map( minMortality, maxMortality, 0f, VISUALIZATION_SIZE, getFloat(d, "childMortality", year))))
				.attr("z", (d,i) => Mathf.Log(d4.map( minFertility, maxFertility, 0f, VISUALIZATION_SIZE, getFloat(d, "fertility", year))))
